...
int main(int argn, char *argv[])
{
	...
	const VECTOR neighbor_grid_box_length = {400.f, 400.f, 400.f}; // 锟斤拷住锟斤拷锟斤拷模锟斤拷原锟接的空硷拷锟叫★拷锟斤拷徒锟斤拷诒锟斤拷锟斤拷锟斤拷锟截ｏ拷
	const float cutoff = 8.f;									   // 锟截讹拷
	...
	unsigned int stream_numbers = 384; // 锟矫版本每锟斤拷stream锟酵讹拷应一锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
	unsigned int search_depth = 40;	   // 每锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟侥达拷锟斤拷
	...
	for (int task_i = 0; task_i < task_io_list.size(); task_i++)
	{
		...
		int optim_kernel_version[stream_numbers]; // 锟芥定锟脚伙拷kernel: 0/1
		...
		while (true)
		{
			...
			for (int i = 0; i < stream_numbers; i = i + 1)
			{
				if (task[i].Is_empty()) // 锟斤拷锟斤拷锟角皊tream锟斤拷锟斤拷锟斤拷
				{

					if (task[i].Get_Status() == DSDP_TASK_STATUS::MINIMIZE_STRUCTURE) // 锟斤拷锟斤拷锟角皊tream锟斤拷锟斤拷锟斤拷锟斤拷一锟斤拷锟斤拷锟脚伙拷
					{

						/* METROPOLIS ACCEPT/REJECT */
						// 锟斤拷锟斤拷锟斤拷dof_flex-1锟斤拷锟絠nter energy
		...
		// 锟斤拷锟斤拷锟斤拷total_energy 锟斤拷锟斤拷慕锟斤拷锟斤拷为锟斤拷锟斤拷锟斤拷锟斤拷锟�
		...
		for (int i = 0; i < DSDP_sort.selected_numbers; i += 1)
		{
			...
			// 锟斤拷锟斤拷锟睫改碉拷copy_pdbqt锟斤拷识锟斤拷HETATM
