...
void NEIGHBOR_GRID::Put_Atom_Into_Grid_Bucket(int atom_numbers, VECTOR *crd)
{
	for (int i = 0; i < atom_numbers; i = i + 1)
	{
		...
		if (temp_serial.int_x < 0 || temp_serial.int_x >= grid_dimension.int_x || temp_serial.int_y < 0 || temp_serial.int_y >= grid_dimension.int_y || temp_serial.int_z < 0 || temp_serial.int_z >= grid_dimension.int_z)
		{
		...
		neighbor_grid_bucket[grid_serial].atom_serial[neighbor_grid_bucket[grid_serial].atom_serial[0]] = i;
