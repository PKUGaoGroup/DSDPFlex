...
int main(int argn, char *argv[])
{
	...
	char ligand_name[MAXLINE];	// 必须输入
	char protein_name[MAXLINE]; // 必须输入
	...
	CLI::App app{"DSDP"};
	std::string ligand_string, protein_string, flex_string = "None";
	std::vector<std::string> ligand_list;
	...
	app.description("DSDP: Deep Site and Docking Pose\n"
					"  This is the re-docking build, docking with known binding site.\n"
					"  See more details at https://github.com/PKUGaoGroup/DSDP\n");

	app.add_option("--ligand", ligand_string, "ligand input PDBQT file / PDBQT list [REQUIRED]")
		->option_text("<pdbqt>/<txt>")
		->required()
	...
	app.add_option("--kernel_type")
		...
		->group("Search settings");
	app.add_flag("--randomize_only", randomize_only);
	app.add_flag("--no_norm", no_normalization);
	app.add_flag("--rescore", rescore);
	app.add_flag("--rank_ligand_only", rank_only_ligand);
	app.add_option("--norm_param", FLEX_RATIO_PAR, "flex_ratio = norm_param * min(g_ligand/f_flex, 1)")->default_val(0.5);
	...
	time(&now_time);
	printf(
		"|\\(`|\\|)  DSDPFlex: An Improved Flexible-Receptor\n"
		"|/_)|/|   Docking Method with GPU Acceleration\n");
	printf("START  > %s", ctime(&now_time));
	if (flex_string != "None")
		flex_mode = true;

	// single mode or batch mode?
	std::string ligand_suffix_str = ligand_string.substr(ligand_string.find_last_of('.') + 1);
	if (ligand_suffix_str == "pdbqt") // single ligand
	...
	sscanf(out_flex_string.c_str(), "%s", fsc_out_pdbqt_name);
	...
	printf("* protein: %s\n* ligand:  %s\n* flex:    %s\n", protein_name, ligand_name, protein_flex_name);
	...
	// FIXME too small ? this is similar to Vina
	rotational_amp = 2.f / ligand[0].gyration_radius;
	...
	copy_pdbqt.Initial(ligand_name); // for output
	...
	while (true)
	{
		...
		for (int i = 0; i < stream_numbers; i = i + 1)
		{
			if (task[i].Is_empty()) // 如果当前stream无任务
			{
				...
				New_Total_Optimization_Kernel<<<1, 128, sizeof(float) * 46, task[i].Get_Stream()>>>(
					// protein grid
	...
	DSDP_sort.Sort_Structures(
		...
		rmsd_similarity_cutoff, SORT_NUM, SORT_NUM);
	...
	for (int i = 0; i < DSDP_sort.selected_numbers; i = i + 1)
	{
		for (int j = 0; j < copy_numbers; j = j + 1)
		{
			...
			{
				ligand[stream_serial].Refresh_origin_crd(&DSDP_sort.selected_lig_crd[(size_t)i * ligand[stream_serial].atom_numbers]);
				ligand[stream_serial].vina_gpu.Initial(&ligand[stream_serial], &ligand[stream_serial].vina_tree);
				flexsc[stream_serial].Refresh_origin_crd(&DSDP_sort.selected_fsc_crd[(size_t)i * flexsc[stream_serial].atom_numbers]);
				flexsc[stream_serial].vina_gpu.Initial(&flexsc[stream_serial], &flexsc[stream_serial].vina_tree);
			}
		}
	}
	// 第二次搜索
	printf("running second search ... ");
	...
	for (int i = 0; i < stream_numbers; i = i + 1)
	{
		ligand[i].vina_gpu.h_u_crd[dof_lig - 6] = ligand[i].move_vec.x;
		ligand[i].vina_gpu.h_u_crd[dof_lig - 5] = ligand[i].move_vec.y;
		ligand[i].vina_gpu.h_u_crd[dof_lig - 4] = ligand[i].move_vec.z;
		flexsc[i].vina_gpu.h_u_crd[dof_flex - 6] = flexsc[i].move_vec.x;
		flexsc[i].vina_gpu.h_u_crd[dof_flex - 5] = flexsc[i].move_vec.y;
		flexsc[i].vina_gpu.h_u_crd[dof_flex - 4] = flexsc[i].move_vec.z;
		// last accepted remove
		ligand[i].vina_gpu.last_accepted_energy = 1000.f;
		flexsc[i].vina_gpu.last_accepted_energy = 1000.f;
		flexsc[i].vina_gpu.last_accepted_inter_energy = 1000.f;
		task[i].Assign_Status(DSDP_TASK_STATUS::EMPTY);
		memcpy(ligand[i].vina_gpu.h_last_accepted_u_crd, ligand[i].vina_gpu.h_u_crd, sizeof(float) * dof_lig);
		memcpy(flexsc[i].vina_gpu.h_last_accepted_u_crd, flexsc[i].vina_gpu.h_u_crd, sizeof(float) * dof_flex);
	}
	...
	//            or = ligand_energy
	energy_record_ranking = total_energy_record;
	...
	if (rescore)
	{
		...
		Rescore rescore_worker;
		rescore_worker.initialize_from_input(protein_string, out_pdbqt_name, out_flex_string);

		for (int i = 0; i < desired_saving_pose_numbers; i += 1)
		{
			FILE *fligand = fopen(rescore_worker.temp_ligand_out.c_str(), "w");
			FILE *fflex = fopen(rescore_worker.temp_flex_out.c_str(), "w");
			copy_pdbqt.Append_Frame_To_Opened_pdbqt(fligand, &lig_crd_record[(size_t)i * (ligand[0].atom_numbers)], move_vec);
			copy_pdbqt_flex.Append_Frame_To_Opened_pdbqt(fflex, &fsc_crd_record[(size_t)i * (flexsc[0].atom_numbers)], move_vec);
			fclose(fligand);
			fclose(fflex);
			float gnina_score = rescore_worker.call_gnina();
	...
	FILE *ligand_out_pdbqt = fopen(out_pdbqt_name, "w");  // file recording poses
	FILE *fsc_out_pdbqt = fopen(fsc_out_pdbqt_name, "w"); // file recording poses
	FILE *out_list = fopen(out_list_name, "w");			  // output log file
	if (!ligand_out_pdbqt || !fsc_out_pdbqt || !out_list)
	{
		perror("DSDP is unable to open files");
		return 1;
	}

	// printf("size of crd %d  of energy %d\n", fsc_crd_record.size(), fsc_energy_record.size());
	...
	for (int i = 0; i < DSDP_sort.selected_numbers; i += 1)
	{
		...
		copy_pdbqt.Append_Frame_To_Opened_pdbqt_standard(
			ligand_out_pdbqt,
			&DSDP_sort.selected_lig_crd[(size_t)i * (ligand[0].atom_numbers)],
	...
	fprintf(out_list, "Total time %lf s\n", omp_get_wtime() - time_begin);
