...
void PARTIAL_RIGID_SMALL_MOLECULE::Initial_From_PDBQT(const char *file_name)
{
	...
	while (true)
	{
		...
		else if (strcmp(str_segment, "BRANCH") == 0)
		{
			...
			while (fgets(str_line, 256, in)) // changed 7.6
			{
				sscanf(str_line, "%s", str_segment);
	...
	for (int i = 0; i < atom_numbers; i++)
	{
		if (atomic_number[i] == 1)
			continue;
		float vec_distance_sqr = v_inner_product(crd[i], crd[i]);
		acc += vec_distance_sqr;
		heavy_counter += 1;
	}
	gyration_radius = sqrt(acc / heavy_counter);
	// end of gyration_radius
	...
	pdbqt_gpu.last_accepted_energy = 100.f;
	// 10.26 support pdbqt_gpu
...
void PARTIAL_RIGID_SMALL_MOLECULE::GPU::Initial(PARTIAL_RIGID_SMALL_MOLECULE *mol, const TREE *tree)
{
	...
	else
	{
		if (origin_crd != NULL)
		{
			cudaFree(origin_crd);
			cudaFree(ref_crd);
			cudaFree(stored_ref_crd);
			cudaFreeHost(h_ref_crd);
			cudaFreeHost(h_stored_ref_crd);
			cudaFree(crd);
			cudaFree(last_crd);
			cudaFree(frc);
		...
		cudaMalloc((void **)&stored_ref_crd, sizeof(VECTOR) * atom_numbers);
		...
		// ref_crd 相关
		cudaMallocHost((void **)&h_ref_crd, sizeof(VECTOR) * atom_numbers);
		cudaMallocHost((void **)&h_stored_ref_crd, sizeof(VECTOR) * atom_numbers);
	...
	else
	{
		...
		{
			cudaFree(node);
			cudaFree(stored_node);
			cudaFreeHost(h_node);
			cudaFreeHost(h_stored_node); // added
		}
		cudaMalloc((void **)&node, sizeof(NODE) * node_numbers);
		cudaMalloc((void **)&stored_node, sizeof(NODE) * node_numbers);
		cudaMallocHost((void **)&h_node, sizeof(NODE) * node_numbers);
		cudaMallocHost((void **)&h_stored_node, sizeof(NODE) * node_numbers);
	...
	cudaMemcpy(stored_ref_crd, &partial_rigid_small_molecule[0].origin_crd[0], sizeof(VECTOR) * atom_numbers, cudaMemcpyHostToDevice);
	...
	// ref_crd 相关
	memcpy(h_ref_crd, &partial_rigid_small_molecule[0].origin_crd[0], sizeof(VECTOR) * atom_numbers);
	memcpy(h_stored_ref_crd, &partial_rigid_small_molecule[0].origin_crd[0], sizeof(VECTOR) * atom_numbers);

	cudaMemcpy(node, &tree[0].node[0], sizeof(NODE) * node_numbers, cudaMemcpyHostToDevice);
	cudaMemcpy(stored_node, &tree[0].node[0], sizeof(NODE) * node_numbers, cudaMemcpyHostToDevice);
	memcpy(h_node, &tree[0].node[0], sizeof(NODE) * node_numbers); // added
}
...
void PARTIAL_RIGID_SMALL_MOLECULE::Copy_From_PARTIAL_RIGID_SMALL_MOLECULE(PARTIAL_RIGID_SMALL_MOLECULE *input)
{
	...
	gyration_radius = input[0].gyration_radius;
...
void PARTIAL_RIGID_SMALL_MOLECULE::GPU::update_nodes(VECTOR *atoms_crd)
{

	// NODE *h_node = new NODE[node_numbers]; // make new node
	// cudaMemcpy 放在外部
	// cudaMemcpy(h_node, this->node, sizeof(NODE) * node_numbers, cudaMemcpyDeviceToHost);
	// memcpy(this->h_stored_node, h_node, sizeof(NODE) * node_numbers);
	for (int i = 0; i < node_numbers; i++)
	{
		// h_node is based on node. only do neccessary changing here
		// root->branch
		int &from_atom = h_node[i].root_atom_serial;
		int &to_atom = h_node[i].branch_atom_serial;

		h_node[i].a0 = atoms_crd[from_atom];
		h_node[i].n0 = {atoms_crd[to_atom].x - atoms_crd[from_atom].x,
						atoms_crd[to_atom].y - atoms_crd[from_atom].y,
						atoms_crd[to_atom].z - atoms_crd[from_atom].z};
		...
		float length = 1.f / sqrtf(v_inner_product(h_node[i].n0, h_node[i].n0));
		// normalization
		h_node[i].n0.x *= length;
		h_node[i].n0.y *= length;
		h_node[i].n0.z *= length;
		// shift
		h_node[i].a0.x -= atoms_crd[0].x;
		h_node[i].a0.y -= atoms_crd[0].y;
		h_node[i].a0.z -= atoms_crd[0].z;

		h_node[i].a = h_node[i].a0;
		h_node[i].n = h_node[i].n0;
	}

	// cudaMemcpy(this->node, h_node, sizeof(NODE) * node_numbers, cudaMemcpyHostToDevice);
	return;
}
// change h_ref_crd, h_node
void PARTIAL_RIGID_SMALL_MOLECULE::GPU::update_ref_structure()
{
	// printf("----\n");
	// print_vec(v_minus(h_ref_crd[1], h_ref_crd[0]), "old 0->1");
	// print_vec(v_minus(h_ref_crd[9], h_ref_crd[8]), "old 8->9");
	// print_vec(v_minus(h_ref_crd[14], h_ref_crd[11]), "old 11->14");
	float rot_matrix[9];
	std::vector<VECTOR> atoms_crd;
	atoms_crd.resize(atom_numbers);
	// 在当前广义坐标下更新各转动矩阵
	float cos_c;
	float sin_c;
	float cos_b;
	...
	float cos_a;
	float sin_a;
	sincosf(h_u_crd[u_freedom - 3], &sin_c, &cos_c);
	sincosf(h_u_crd[u_freedom - 2], &sin_b, &cos_b);
	sincosf(h_u_crd[u_freedom - 1], &sin_a, &cos_a);
	...
	for (int i = 0; i < atom_numbers; i++)
	{
		VECTOR temp_crd1 = h_ref_crd[i];
		VECTOR temp_crd2;
		const VECTOR center = h_ref_crd[0];
		...
		// ref_crd更新
		atoms_crd[i].x = temp_crd2.x;
		atoms_crd[i].y = temp_crd2.y;
		atoms_crd[i].z = temp_crd2.z;
	}

	for (int i = 0; i < atom_numbers; i++)
	{
		h_ref_crd[i] = atoms_crd[i];
	}
	// print_vec(v_minus(h_ref_crd[1], h_ref_crd[0]), "new 0->1");
	// print_vec(v_minus(h_ref_crd[9], h_ref_crd[8]), "new 8->9");
	// print_vec(v_minus(h_ref_crd[14], h_ref_crd[11]), "new 11->14");
	for (int i = 0; i < node_numbers; i++)
	{
		...
		int &to_atom = h_node[i].branch_atom_serial;
		// printf("node #%d   %d->%d\n", i, from_atom, to_atom);
		// print_vec(h_node[i].a0, "a0 old");
		// print_vec(h_node[i].n0, "n0 old");
		h_node[i].a0 = atoms_crd[from_atom];
		h_node[i].n0 = {atoms_crd[to_atom].x - atoms_crd[from_atom].x,
						atoms_crd[to_atom].y - atoms_crd[from_atom].y,
						atoms_crd[to_atom].z - atoms_crd[from_atom].z};
		...
		// print_vec(h_node[i].a0, "a0 new");
		// print_vec(h_node[i].n0, "n0 new");
	}
	return;
}
