...
{
	float theta = range * rand() / RAND_MAX;
	// 生成随机方向矢量
...
static void Refresh_Atom_Type_For_Vina_Atom(const int atom_numbers, VINA_ATOM *vina_atom)
{
	for (int i = 0; i < atom_numbers; i = i + 1)
	{
		...
		else if (vina_atom[i].radius == -1.f)
		{
...
/* something help to calc dihedral*/
...
VECTOR v_minus(VECTOR a, VECTOR b)
{
	return VECTOR{a.x - b.x, a.y - b.y, a.z - b.z};
}
...
// given A->B->C->D, return the dihedral
float calc_dihedral(VECTOR ab, VECTOR bc, VECTOR cd)
{
	double norm2_bc = v_inner_product(bc, bc);
	double norm2_ab = v_inner_product(ab, ab);
	double norm2_cd = v_inner_product(cd, cd);
	VECTOR ab_p = v_times(bc, -1.0 * v_inner_product(ab, bc) / norm2_bc);
	VECTOR ab_v = v_plus(ab, ab_p);
	VECTOR cd_p = v_times(bc, -1.0 * v_inner_product(cd, bc) / norm2_bc);
	VECTOR cd_v = v_plus(cd, cd_p);
	VECTOR ba_v = {-ab_v.x, -ab_v.y, -ab_v.z};
	double cos_theta = v_inner_product(ba_v, cd_v) / (sqrt(norm2_ab) * sqrt(norm2_cd));
	double theta = acos(cos_theta);
	// Cross product
	VECTOR v_xp = {ba_v.y * cd_v.z - ba_v.z * cd_v.y,
				   ba_v.z * cd_v.x - ba_v.x * cd_v.z,
				   ba_v.x * cd_v.y - ba_v.y * cd_v.x};
	if (v_inner_product(bc, v_xp) > 0)
	{
		return theta;
	}
	else
	{
		return 2 * 3.141592654f - theta; // DSDP sampling version
										 // if standard definittion, return -theta
	}
}
...
// return 2.f * 3.141592654f * ((float)rand() / RAND_MAX)
float random_2pi()
{
	return 2.f * 3.141592654f * ((float)rand() / RAND_MAX);
}
// return 1.f * (2.f * ((float)rand() / RAND_MAX) - 1.f)
float random_2A()
{
	return 1.f * (2.f * ((float)rand() / RAND_MAX) - 1.f);
}
						...
						std::vector<TaskIO> *task_io_list)
{
	...
	char buf[1024];
	char lig_in[256] = "";
	char lig_out[256] = "", fsc_out[256] = "";
	FILE *in = fopen_safely(file_name.c_str(), "r");
	while (fgets(buf, 1024, in))
	{
		if (strcmp(buf, "\n") == 0) // ignore \n
			continue;
		sscanf(buf, "%s %s %s", lig_in, lig_out, fsc_out);
		if (lig_in == "" || lig_out == "" || fsc_out == "")
		{
			printf("invalid input! <ligand> <out> <out_flex>\n");
		}
		TaskIO this_task = {lig_in, lig_out, fsc_out};
		task_io_list->push_back(this_task);
	}
	if (task_io_list->size() < 1)
		printf("no valid task in %s?\n", file_name.c_str());
	int task_length = task_io_list->size();
	return task_length;
}